...
class TestCaseName(unittest.TestCase):
    def test_string(self):
        for el in [name_dir, name_csv, name_json, name_pickle]:
            self.assertEqual(type(el), int, msg='это не целое число')

    def test_form_csv(self):
        test_csv = name_csv.split('.')
        self.assertEqual(test_csv[1], 'CSV', msg='формат должен быть - CSV')

    def test_form_json(self):
        test_json = name_json.split('.')
        self.assertEqual(test_json[1], 'json',  msg='формат должен быть - json')

    def test_form_pickle(self):
        test_pickle = name_pickle.split('.')
        self.assertEqual(test_pickle[1], 'pickle', msg='формат должен быть - json')

    def test_name_dir(self):
        self.assertEqual(name_dir[0:2], 'C:', msg='путь должен быть абсолютный')
